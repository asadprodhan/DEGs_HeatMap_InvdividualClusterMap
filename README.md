<h1 align="center">DEG Hierarchical Clustering Heatmap and Individual Cluster Plots</h1>
...
- Make the heatmap of the clusters
...
**Q1. How do I handle the zero values in my gene expression matrix?**

DESeq2, the bioinformatics tool used for differentially expressed gene analysis, uses a method called Variance Stabilizing Transformation (VST). The VST formula involves an adjustment where low counts (including zeros) are transformed to nonzero values to stabilize variance. Effectively, all zero counts become small nonzero values after transformation.

**Q2. After vst transformation, I performed z-score transformation. Then some NA were introduced. Why?**

The NA values after Z-score transformation in your VST-transformed data likely occurred due to constant expression levels (zero variance) across samples for some genes. Here's why:
...
X = individual gene expression value

μ = mean expression across samples for that gene

σ = standard deviation (SD) across samples for that gene
